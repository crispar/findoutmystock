...
def make_request(url):
    ...
    try:
        with urlopen(req, context=ssl._create_unverified_context(), timeout=10) as response:
            # The KOSDAQ page uses 'euc-kr' but the KOSPI one doesn't, so we decode here based on URL
            encoding = 'euc-kr' if 'sosok=1' in url else 'utf-8'
...
def top_rank_stocks(num, top_ranks, page_n=1, last_page=0):
    ...
    url_float = f'https://finance.naver.com/sise/sise_market_sum.naver?sosok=1&page={str(page_n)}' # KOSDAQ URL
    soup = make_request(url_float)
    ...
    for stock_link in stock_table.find_all('a', class_='tltle'):
        ...
        if not stock_name_text or any(k in stock_name_text for k in ['스팩', 'KODEX', 'TIGER']):
            continue
...
# --- Main Execution ---
print("Fetching top 3 KOSDAQ stocks for verification...")
